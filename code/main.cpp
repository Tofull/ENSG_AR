...
int main() {
  ...
  GLFWwindow *window = glfwCreateWindow(screenWidth, screenHeight, "OpenGL",
  ...
  Shader shader("opengl_code/shaders/default.vertexshader",
                "opengl_code/shaders/default.fragmentshader");
  Shader lightshader("opengl_code/shaders/light.vertexshader",
                     "opengl_code/shaders/light.fragmentshader");
  ...
  // Creation de la perspective
  projection =
      glm::perspective(0.7348556912801973f, 640.0f / 480.0f, 0.5f, 100.0f);

  float focale = projection[0][0];
  // Matrice de position de la caméra
  glm::mat4 view =
      glm::lookAt(glm::vec3(0.0f, 0.0f, 0.5 * focale),
  ...
  lightshader.Use();
  glUniformMatrix4fv(glGetUniformLocation(lightshader.Program, "view"), 1,
                     GL_FALSE, glm::value_ptr(view));
  glUniformMatrix4fv(glGetUniformLocation(lightshader.Program, "projection"), 1,
                     GL_FALSE, glm::value_ptr(projection));

  /*********************************************************************/

  Model monObjet = Model("opengl_code/model/suzanne/suzanne.obj");
  Model maMaison = Model("opengl_code/model/nanosuit/nanosuit.obj");
  std::vector<TagData> listTag;
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glBindTexture(
        GL_TEXTURE_2D,
        texture); //  Association du numero de la texture  pour le  shader
    ...
    glClear(GL_DEPTH_BUFFER_BIT);
    lightshader.Use();
    listTag = reader.getTags();

    for (size_t i = 0; i < listTag.size(); i++) {
      if (listTag[i].tagId == 0) {

        model = glm::mat4(1.0);
        std::cout << "modele modifie" << std::endl;

        model[3][0] = listTag[i].position[0];
        model[3][1] = listTag[i].position[1];
        model[3][2] = listTag[i].position[2];

        for (size_t j = 0; j < 3; j++) {
          for (size_t k = 0; k < 3; k++) {
            model[j][k] = listTag[i].orientation[k][j];
          }
        }

        model = glm::scale(model, glm::vec3(5.0f, 5.0f, 5.0f));
        model =
            glm::rotate(model, glm::pi<float>(), glm::vec3(0.0f, 0.0f, 1.0f));
        // On passe la position de l'objet au shader
        glUniformMatrix4fv(glGetUniformLocation(lightshader.Program, "model"),
                           1, GL_FALSE, glm::value_ptr(model));

        monObjet.Draw(lightshader);
      }
      if (listTag[i].tagId == 1) {
